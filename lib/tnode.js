...
class TNode {
    ...
    _parseInter() {
        ...
        res.forEach(group => {
            ...
            let nextElem = (!this._node.nextSibling) ? null :
                Path.fromNode(this._node.nextSibling);
     ...
     */
    _renderAttr(node) {
        if (!this.isAttr) return new TypeError("TNode referenced has not any attribute", "tnode.js");
        this.fields.forEach(field => node.setAttribute(field.attr, field.value));
    }
     ...
     */
    _renderInter(node) {
        if (!this.isInterpolation) return new TypeError("TNode referenced is not an interpolation", "tnode.js");

        let [index, field] = Array.from(this.fields.entries())[0];
    ...
    _clearInter(elem, nextElem) {
        if (!elem || elem.nodeType === undefined) {
            return new TypeError("The argument provided is not a Node", "tnode.js");
        }
